...
- `patterns/hero.php` — PHP pattern (slug `indexworld/hero`, category `indexworld`). **Centered** layout: H1 "We implement Odoo ERP…", centered subtext, then two buttons — "Let's Talk business" (dark navy `.ix-hero__btn--dark`) + "Odoo Partner" (white `.ix-hero__btn--white`). Below that, a FULL-WIDTH `wp:html` carousel.
