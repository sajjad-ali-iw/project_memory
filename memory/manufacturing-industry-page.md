...
- **WP's old-slug 301 does NOT fire for pages** (core limitation; `_wp_old_slug` meta was set anyway, no effect). Fixed via `functions.php` `$ix_canon` map (`'supply-chain-manufacturing' => 'manufacturing-industry'`) — **deployed 2026-07-11, commit `d29b6d7`**, verified 301. Same commit also 301s `/multi-channel-ecommerce-2/` → `/multi-channel-ecommerce/` (page 912 slug drift: stale drafts 911/930 held the base slug → renamed to `-draft-<id>` per the existing convention, page reclaimed the clean slug). **Slug-collision lesson: drafts DO hold page slugs — check drafts (`wp_get_posts status:any`) when a page sprouts a `-2`.**
