...
- **Shared server:** ChemiCloud — subdomain `prelive.indexworld.net` as shared dev/prelive environment
...
**Workflow:** Each dev clones the theme, pulls before starting, pushes after changes. Content (pages/media) managed by one person at a time via MCP with clear communication. Full plan doc at `docs/COLLABORATION.md` in the theme.

**Why:** Multiple developers can't work simultaneously on one Local by Flywheel instance.
**How to apply:** Before pushing any changes, always `git pull origin main` first. After Claude saves a memory file, commit `project_memory/` and push to keep it in sync.
